...
int main()
{
    ...
    int n, m, k, t;
    cin >> n >> m >> k >> t;
    string prod[3]{"Carrots", "Kiwis", "Grapes"};
    set<int> waste;
    forn(i, k)
    {
        int x, y;
    ...
    forn(i, t)
    {
        ...
        cin >> x >> y;
        int num = (x - 1) * m + y;
        if (waste.find(num) != waste.end())
        {
        }
        else
        {
            int nb_waste_before_num = lower_bound(all(v), num) - v.begin();
            println(prod[(num - 1 - nb_waste_before_num) % 3]);
        }
    }
