        ...
        forn(i, n)
        {
            forn(j, n)
            {
                if (grid[i][j] == 'x' && !visited[i][j])
                {
                    ...
                    visited[i][j] = true;
                    int x = j + 1;
                    while (valid(i, x) && x - j + 1 <= n / 2 && (grid[i][x] == 'x' || grid[i][x] == '@') && !visited[i][x])
                    {
                        visited[i][x] = true;
                        x++;
                    }
                    if (x == j + 1) // Not a horizontal ship, check if it's a vertical ship
                    {
                        int y = i + 1;
                        while (valid(y, j) && y - i + 1 <= n / 2 && (grid[y][j] == 'x' || grid[y][j] == '@') && !visited[y][j])
                        {
                            visited[y][j] = true;
                            y++;
                        }
