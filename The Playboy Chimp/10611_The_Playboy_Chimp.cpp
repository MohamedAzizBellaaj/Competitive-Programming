...
int binary_search_upper(v32 &v, int x)
{
    int left = 0, right = sz(v) - 1, middle;
    while (left <= right)
    {
    ...
    return left >= sz(v) ? -1 : v[left];
}

int binary_search_lower(v32 &v, int x)
{
    int left = 0, right = sz(v) - 1, middle;
    while (left <= right)
    {
...
int main()
{
    ...
    while (q--)
    {
        cin >> x;
        {
            first = binary_search_lower(v, x), second = binary_search_upper(v, x);
            if (first == -1)
            {
                print("X ");
            }
