...
int main()
{
    ...
    v32 v(n);
    cin >> v;
    auto it_max = find(all(v), *max_element(all(v)));
    int id_max = it_max - v.begin();
    auto min_it = find(all(v), *min_element(all(v)));
    int id_min = min_it - v.begin();
    v32 positions(4);
    positions[0] = id_min;
    positions[1] = sz(v) - id_min - 1;
    positions[2] = id_max;
    positions[3] = sz(v) - id_max - 1;
    print(*max_element(all(positions)));
