...
int main()
{
    ...
    forn(i, n)
    {
        forn(j, m)
        {
            ...
            forn(kx, x + 1)
            {
                forn(ky, x + 1)
                {
                    vp32 dir = {{0, ky}, {kx, 0}, {-kx, 0}, {0, -ky}, {-kx, ky}, {kx, -ky}, {kx, ky}, {-kx, -ky}};
                    for (auto const &[dx, dy] : dir)
                    {
                        if (valid(i + dx, j + dy) && grid[i + dx][j + dy] == 'S' && distance({i, j}, {i + dx, j + dy}) <= x * x)
                        {
                            mp[grid[i][j]] = 0;
                            break;
                        }
    ...
    for (char const &c : t)
    {
        if (mp.find(tolower(c)) == mp.end() || (isupper(c) && mp.find(tolower(c)) != mp.end() && mp.find('!') == mp.end()))
        {
            out = -1;
            break;
        }
