...
int main()
{
    ...
    {
        bool fits = false;
        forn(j, sz(rectangles))
        {
            if (points[i].fi > rectangles[j].fi.fi && points[i].fi < rectangles[j].se.fi && points[i].se > rectangles[j].se.se && points[i].se < rectangles[j].fi.se)
            {
                fits = true;
                println("Point", i + 1, "is contained in figure", j + 1);
            }
        }
        if (!fits)
        {
