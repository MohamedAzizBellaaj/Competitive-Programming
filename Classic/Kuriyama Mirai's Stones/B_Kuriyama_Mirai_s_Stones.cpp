...
int main()
{
    ...
    sort(all(v));
    v64 sorted_prefix_arr(n);
    sorted_prefix_arr[0] = v[0];
    forsn(i, 1, n)
    {
        sorted_prefix_arr[i] = v[i] + sorted_prefix_arr[i - 1];
    }
    int m;
    cin >> m;
    while (m--)
    {
        ...
        if (type == 1)
        {
            if (l == 1)
            {
                println(prefix_arr[r - 1]);
            }
            else
            {
                println(prefix_arr[r - 1] - prefix_arr[l - 2]);
            }
        }
        else
        {
            if (l == 1)
            {
                println(sorted_prefix_arr[r - 1]);
            }
            else
            {
                println(sorted_prefix_arr[r - 1] - sorted_prefix_arr[l - 2]);
            }
        }
    }
