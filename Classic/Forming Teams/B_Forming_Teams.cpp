...
status dfs(int cur_node, int parent_node, int &nodes_count)
{
    if (visited[cur_node])
    {
        return CYCLE;
    ...
    for (auto const &child_node : graph[cur_node])
    {
        if (child_node != parent_node)
        {
            nodes_count++;
            if (dfs(child_node, cur_node, nodes_count) == CYCLE)
            {
                return CYCLE;
            }
        }
...
int main()
{
    ...
    forn(i, n)
    {
        ...
        {
            int count = 0;
            status solve = dfs(i, -1, count);
            if (solve == CYCLE && count % 2 == 1)
            {
                out++;
            }
        }
