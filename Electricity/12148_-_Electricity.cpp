...
bool endOfMonth(date a)
{
    ...
    else if ((a.y % 4 == 0 && a.y % 100 != 0) || a.y % 400 == 0)
    {
        return a.d == 29;
    }
    else
...
bool endOfYear(date a)
{
    return a.m == 12 && endOfMonth(a);
}
int main()
{
    fast_cin();
    int n, count, consumption;
    while (cin >> n && n != 0)
    {
        count = 0;
        consumption = 0;
        ...
        forn(i, n - 1)
        {
            if (endOfYear(v[i]) && v[i + 1].d == 1 && v[i + 1].m == 1 && (v[i + 1].y - v[i].y) == 1)
            {
                count++;
                consumption += v[i + 1].c - v[i].c;
            }
            else if (v[i].y == v[i + 1].y)
            {
                ...
                else if (endOfMonth(v[i]) && v[i + 1].d == 1 && (v[i + 1].m - v[i].m) == 1)
                {
                    count++;
                    consumption += v[i + 1].c - v[i].c;
                }
