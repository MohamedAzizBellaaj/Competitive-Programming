...
bool canPlace(v32 &v, int c, int distance)
{
    int current_val = v[0], ptr = 1, count = 1;
    forsn(i, 1, sz(v))
    {
        if (v[i] - current_val >= distance)
        {
            count++;
            current_val = v[i];
        }
...
int binarySearch(v32 &v, int c)
{
    ...
    return canPlace(v, c, left) ? left : right;
}
