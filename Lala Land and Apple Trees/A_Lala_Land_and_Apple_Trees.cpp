...
bool how_pos(p32 a, p32 b)
{
    return a.fi < b.fi;
}

bool how_neg(p32 a, p32 b)
{
    return a.fi > b.fi;
}
int main()
{
    ...
    sort(all(pos), how_pos);
    sort(all(neg), how_neg);
    int out = 0, counter = 0, length = min(sz(pos), sz(neg));
    for (; counter < length; counter++)
    {
        out += pos[counter].se + neg[counter].se;
    }
    if (sz(pos) > sz(neg))
    {
        out += pos[counter].se;
