...
int main()
{
    ...
    tt
    {
        ...
        cin >> n >> q;
        v32 strengths(n);
        vp32 extremes(n, {INT_MAX, 0});
        cin >> strengths;
        int biggest = 0, fight = 1, fighter = strengths[0];
        forsn(i, 1, n)
        {
            if (strengths[i] < fighter)
            {
                if (extremes[biggest].fi == INT_MAX)
                {
                    extremes[biggest].fi = fight;
                }
                extremes[biggest].se++;
            }
            else
        ...
        while (q--)
        {
            ...
            {
                println(min(extremes[i].se, k - extremes[i].fi + 1));
            }
        }
    }
